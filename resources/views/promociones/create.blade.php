...
<div class="card">
    ...
    <form method="POST" action="{{ route('promociones.store') }}" id="form_promocion">
        ...
        <div class="card" style="background: #FAFAFA; margin-bottom: 22px;">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="tipo" id="tipo_promocion" required>
                        ...
                        <option value="por_vencimiento" @selected(old('tipo') === 'por_vencimiento')>Por vencimiento</option>
                    </select>
                </div>
            ...
            <div class="form-group">
                <label>Motivo</label>
                <input type="text" name="motivo" value="{{ old('motivo') }}" placeholder="Ej. Producto por vencer, campaña, oferta especial">
            </div>
        </div>
    ...
    resultadosPromocion.innerHTML = productos.map(producto => {
        ...
        return `
            <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; padding:12px; border-bottom:1px solid #E5E7EB;">
                ...
                <button type="button" class="btn-primary" onclick='agregarItemPromocion(${dataProducto})'>
                    Agregar
                </button>
            </div>
        `;
